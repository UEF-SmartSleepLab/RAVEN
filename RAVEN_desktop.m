    ...
    for i = 1:total_windows
        ...
        points_per_candidate = zeros(size(event_indices));
         ...
         for j = 1:size(event_indices, 1)
            ...
            % % --- Plot window ---
            % win_start = start_index - ext;
            % win_end   = end_index + ext;
            % x = win_start:win_end;
            % 
            % figure(1)
            % clf
            ...
            % % -------- EEG broadband --------
            % subplot(2,1,1)
            % plot(x, EEGbf(win_start:win_end))
            ...
            % % -------- Delta + peaks --------
            % subplot(2,1,2)
            % plot(x, EEG_delta(win_start:win_end))
            % xlim([win_start win_end])
            % ylabel('EEG\_delta')
            % xlabel('Sample index')
            % hold on
            ...
            % 
            % % --- Maxima ---
            % in_win = idx_max >= win_start & idx_max <= win_end;
            % plot(idx_max(in_win), pks_max(in_win), '^r', ...
            %      'MarkerFaceColor', 'r')
            % 
            % % --- Minima (context only) ---
            % in_win = idx_min >= win_start & idx_min <= win_end;
            % plot(idx_min(in_win), pks_min(in_win), 'vr', ...
            %      'MarkerFaceColor', 'r')
            % 
            % hold off
        ...
        for j = 1:size(event_indices, 1)
         ...
         %     threshold = 75; % According to the AASM
         % 
...
for channel = 1 : length(channels_to_analyse)
    ...
    for i = 1:total_windows
        ...
        % ext = 15 * Fs;          % extension for plotting window
